...
import os
#设置页面的标题、图标和布局
st.set_page_config(page_title='企鹅分类器',
                   page_icon='penguin',
                   layout='wide',
)
#使用侧边栏实现多页面显示效果
with st.sidebar:
    st.image(r'D:\streamlit_env\images\rigth_logo.png',width=100)
    st.title('请选择页面')
...
if page =="简介页面":
   ...
   st.header('数据集介绍')
   st.markdown("""帕尔默群岛企鹅数据集是用于数据探索和数据可视化的一个出色的数据集，也可以作为机器学习入门练习。该数据集是由Gorman等收集，
并发布在一个名为palmerpenguins的R语言包,以对南极企鹅种类进行分类和研究。该数据集记录了344行观测数据，包含3个不同物种的企鹅:阿德利企鹅、巴布亚企
鹅和帽带企鹅的各种信息。""")
   st.header('三种企鹅的卡通图像')
   st.image(r'D:\streamlit_env\images\penguins.png')
elif page =="预测分类页面":
   st.header("预测企鹅分类")
...
#使用 pickle 的load方法从磁盘文件反序列化加载一个之前保存的随机森林模型对象
...
with col_logo:
    if not submitted:
        st.image(r'D:\streamlit_env\images\rigth_logo.png', width=300)
    else:
